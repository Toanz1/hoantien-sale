...
export default function ProfilePage() {
 ...
 async function submitPin(event: FormEvent<HTMLFormElement>) {
  ...
  async function saveBank(
    ...
    if (!/^[0-9]{5,30}$/.test(cleanAccountNumber)) {
      setError("Số tài khoản ngân hàng không hợp lệ.");
      return;
    }
    ...
    try {
      ...
      // Thông tin ngân hàng và mã PIN là 2 thiết lập độc lập.
      // Người dùng được phép thêm/cập nhật ngân hàng dù chưa tạo PIN.
      const { data, error: updateError } = await supabase
        .from("profiles")
        .update({
          bank_name: cleanBankName,
          bank_account_number: cleanAccountNumber,
          bank_account_name: cleanAccountName,
        })
        .eq("id", user.id)
        .select(PROFILE_SELECT)
        .single();
      ...
      if (updateError) {
        console.error("Update bank information error:", updateError);
        setError("Không thể lưu thông tin ngân hàng. Vui lòng thử lại.");
        return;
      }
      ...
      showSuccess(
        pinConfigured
          ...
          : "Thông tin tài khoản nhận tiền đã được cập nhật. Bạn có thể thiết lập mã PIN rút tiền bên dưới."
      );
    } catch (err) {
      console.error("Save bank information error:", err);
      setError("Có lỗi kết nối đến hệ thống.");
    } finally {
      setSavingBank(false);
    }
                      ...
                      <div className="flex flex-col gap-3 rounded-2xl bg-gray-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          ...
                          <p className="mt-1 text-xs leading-5 text-gray-400">
                            Mã PIN được thiết lập riêng. Bạn vẫn có thể lưu hoặc cập nhật tài khoản ngân hàng khi chưa có PIN.
                          </p>
                        </div>
                        ...
                        >
                          {pinConfigured ? "✓ Đã thiết lập" : "Chưa thiết lập"}
                        </span>
                      </div>
                    </div>
